...
async def on_message(message):
    ...
    if message.channel.id == int(config['discord_target_channel']):
        ...
        if args[0] == "!pl":
            ...
            embed.add_field(name="鑄造 \u200B \u200B \u200B \u200B", value=f"`{data['total_mint_amount']}`", inline=True)
            embed.add_field(name="買入", value=f"`{data['total_buy_amount']}`")
            embed.add_field(name="賣出", value=f"`{data['total_sell_amount']}`")
            embed.add_field(name="買入價", value=f"`Ξ{round(data['eth_spent'], eth_decimal)} (${data['usd_spent']})`")
            embed.add_field(name="平均買入價", value=f"`Ξ{round(data['eth_avg_buy_price'], eth_decimal)} (${data['usd_avg_buy_price']})`")
            embed.add_field(name="賣出價", value=f"`Ξ{round(data['eth_gained'], eth_decimal)} (${data['usd_gained']})`")
            embed.add_field(name="平均賣出價", value=f"`Ξ{round(data['eth_avg_sell_price'], eth_decimal)} (${data['usd_avg_sell_price']})`")
            embed.add_field(name="持有", value=f"`{data['total_nft_owned']}`")
            embed.add_field(name="估值", value=f"`Ξ{round(data['eth_holding_value'], eth_decimal)} (${data['usd_holding_value']})`")
            ...
            embed.add_field(name="已實現 ROI", value=f"`{data['roi']}%`")
